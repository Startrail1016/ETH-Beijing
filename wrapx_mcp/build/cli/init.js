...
import { group, isCancel, log, multiselect, password, text, } from '@clack/prompts';
...
const TOOLS_WITH_REQUIRED_KEYS = [
    {
        name: 'OpenRouter',
        keys: [
            {
...
░▒▓███████▓▒░░▒▓█▓▒░░▒▓█▓▒░▒▓███████▓▒░░▒▓████████▓▒░      ░▒▓█▓▒░░▒▓█▓▒░░▒▓█▓▒░░▒▓██████▓▒░░▒▓█▓▒░`));
    ...
    log.info('Obtain Coinbase Developer Platform (CDP) API keys at https://portal.cdp.coinbase.com/projects/api-keys.');
    const keys = await group({
        cdpKeyId: () => password({
            message: 'CDP Key ID:',
            validate: (value) => {
                if (!value)
                    return 'Enter a valid CDP Key ID';
                if (!isUuid(value))
        ...
        cdpSecret: () => password({
            ...
            validate: (value) => {
                if (!value)
                    return 'Enter a valid CDP Secret';
            },
        }),
        seedPhrase: () => password({
            message: 'Mnemonic Phrase (optional, will generate a new one if not provided):',
            validate: (value) => {
                if (value) {
