import { AgentKit, basenameActionProvider, cdpApiActionProvider, cdpWalletActionProvider, morphoActionProvider, walletActionProvider, ViemWalletProvider } from '@coinbase/agentkit';
...
import { generateSessionId, getActionProvidersWithRequiredEnvVars, } from './utils.js';
import { version } from './version.js';
export async function main() {
    dotenv.config();
    const apiKeyName = process.env.COINBASE_API_KEY_ID || process.env.COINBASE_API_KEY_NAME; // Previously, was called COINBASE_API_KEY_NAME
    const apiPrivateKey = process.env.COINBASE_API_SECRET || process.env.COINBASE_API_PRIVATE_KEY; // Previously, was called COINBASE_API_PRIVATE_KEY
    ...
    if (!apiKeyName || !apiPrivateKey) {
        console.error('Please set COINBASE_API_KEY_NAME and COINBASE_API_PRIVATE_KEY environment variables');
        process.exit(1);
    }
    // Check if we have a private key or seed phrase
    if (!privateKey && !seedPhrase) {
        console.error('Please set either PRIVATE_KEY or SEED_PHRASE environment variable');
        process.exit(1);
    }
    ...
    if (!chain) {
        throw new Error(`Unsupported chainId: ${chainId}. Only Base and Base Sepolia are supported.`);
    }
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        try {
            ...
            const isBaseMcpTool = baseMcpTools.some((tool) => tool.definition.name === request.params.name);
            if (isBaseMcpTool) {
